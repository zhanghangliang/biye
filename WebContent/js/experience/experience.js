...
function firstcheck() {
	...
	$("#"+id[0]).ajaxSubmit({
		 dataType:"text",
		 error:function(result){alert("firstcheckERROR")},
...
function addForm(){
	...
	$("body").append(' <form id="'+id[i+1]+'" action="/elseCommit" method="post" class="smart-green" enctype="multipart/form-data" onsubmit="return elsecheck();" style="display:none"> <h1 id="blue-h1">摄影经验上传<span>与同一个世界 · 分享 · 不一样的美好</span></h1> <label> <br><span>'+
			number[i]+
			' </span> <input id="name" type="text" name="name" placeholder="给你的作品起个名字吧！" maxlength="14"/> </label> <label> <br> <span>图片素材:</span> <label><label id="filelabel" for="file"><span id="postimg">点击此处上传</span></label><br><br></label> <input type="file" accept="image/*" id="file" name="postimg" style="display: none"/> <div id="image1" style="width:auto;height:auto; background:#CCCCCC;" hidden="hidden"> <center><img id="showspan" height="auto" width="auto" border="5px"/></center> </div> </label> <label> <br><span>图片简介:</span> <textarea id="message" name="message" placeholder="最大字数不超过200字" maxlength="200"></textarea> </label> <center> <br><input name="next" class="blue-sub" type="button" value="结束提交" onclick="end()"><input name="last" class="blue-sub" id="endSub" type="submit" value="下一步"> </center> </form>'
	);
	if(i=="4"){
		$("#endSub").remove();
	}
